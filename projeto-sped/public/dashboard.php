...
// --- CONEXÕES COM MÚLTIPLOS BANCOS ---

/**
 * @uses \DbBusca Classe de busca definida em Db2.php
 * @uses \Db Classe de autenticação definida em Db.php
 */

// 1. CONEXÃO DE USUÁRIOS/AUTENTICAÇÃO (Db.php)
require_once __DIR__ . '/../src/utils/Db.php';
try {
    $pdo_auth = Db::getConnection(); 
} catch (PDOException $e) {
    die("Erro de conexão com o banco de dados de autenticação: " . $e->getMessage());
}

// 2. CONEXÃO DE BLOCOS/REGISTROS (Db2.php)
require_once __DIR__ . '/../src/utils/Db2.php';
try {
    $pdo_search = DbBusca::getConnection(); 
} catch (PDOException $e) {
    die("Erro de conexão com o banco de dados de busca: " . $e->getMessage());
}
...
// ... o resto do seu código dashboard.php (que já usa $pdo_auth e $pdo_search) ...

// Verificando se o usuário está logado
$usuario_logado = isset($_SESSION['usuario_id']) && !empty($_SESSION['usuario_id']);

if ($usuario_logado) {
    // Busca informações do usuário logado (usando $pdo_auth)
    try {
        $stmt = $pdo_auth->prepare("SELECT nome, foto FROM usuarios WHERE id = ?");
...
// Definindo os filtros e preparando para a busca LIKE
$blocos_raw = sanitize_input($_GET['blocos'] ?? '');
$blocos_busca = !empty($blocos_raw) ? '%' . $blocos_raw . '%' : null; 

$registros_raw = sanitize_input($_GET['registros'] ?? '');
$registros_busca = !empty($registros_raw) ? '%' . $registros_raw . '%' : null;
...
$params = [];
$condicoes_ativas = false;
$sql_bloco = "";
$sql_registro = "";
$executar_busca = false; // Flag para controlar a execução

// 1. Consulta para a tabela 'blocos'
if ($blocos_busca !== null) {
    $condicoes_ativas = true;
    $executar_busca = true;
    $sql_bloco = "
        SELECT 
            id, 
            Registro AS Codigo, 
            NomeRegistro AS Descricao, 
            'Blocos' AS Origem 
        FROM 
            blocos 
        WHERE 
...
// 2. Consulta para a tabela 'registro'
if ($registros_busca !== null) {
    $condicoes_ativas = true;
    $executar_busca = true;
    $sql_registro = "
        SELECT 
        ...
        WHERE 
            REG LIKE ?
    ";
    $params[] = $registros_busca;
}

// Combinação das consultas com UNION ALL
if ($condicoes_ativas) {
    $sqls = array_filter([$sql_bloco, $sql_registro]);
    $sql = implode(" UNION ALL ", $sqls);
    $sql .= " ORDER BY Codigo, Origem";
} else {
    // Se não houver filtros, não executa a busca
    $sql = "";
...
if ($executar_busca) {
    try {
        // USA A CONEXÃO DE BUSCA ($pdo_search)
        $stmt = $pdo_search->prepare($sql); 
        $stmt->execute($params);
        $resultados = $stmt->fetchAll(PDO::FETCH_ASSOC);
        $num_resultados = count($resultados);

        // SALVAR NO HISTÓRICO (usa $pdo_auth)
        if ($usuario_logado && $condicoes_ativas) {
            $termo_busca = "Blocos (Registro/NomeRegistro): " . ($_GET['blocos'] ?? '') . 
                          ", Registros (REG): " . ($_GET['registros'] ?? ''); 

            $sql_historico = "INSERT INTO historico_buscas (usuario_id, termo_busca, resultados_encontrados) 
                              VALUES (?, ?, ?)";
            // USA A CONEXÃO DE AUTENTICAÇÃO ($pdo_auth)
            $stmt_historico = $pdo_auth->prepare($sql_historico); 
            $stmt_historico->execute([$_SESSION['usuario_id'], $termo_busca, $num_resultados]);
        }
        
    } catch (PDOException $e) {
        error_log("Erro Fatal na Consulta SQL (UNION): " . $e->getMessage() . " Query: " . $sql);
        $erro_sql = "Ocorreu um erro ao executar a busca: " . $e->getMessage();
    }
}
?>
...
<head>
    ...
    <style>
        /* CSS NOVO PARA DIFERENCIAÇÃO DA ORIGEM */
        .origem-bloco {
            color: #007bff; /* Cor para Bloco (ex: Azul) */
            font-weight: bold;
        }

        .origem-registro {
            color: #28a745; /* Cor para Registro (ex: Verde) */
            font-weight: bold;
        }
    ...
    <main class="center-container">
        ...
        <?php if (isset($erro_sql)): ?>
            <div class="error-message">
                <?= htmlspecialchars($erro_sql) ?>
            </div>
        <?php elseif ($num_resultados > 0): ?>
            <div class="stats">
            ...
            <div class="resultados">
                <?php foreach ($resultados as $resultado): ?>
                    <div class="produto">
                        <h3>
                            Origem: 
                            <span class="<?= $resultado['Origem'] === 'Blocos' ? 'origem-bloco' : 'origem-registro' ?>">
                                <?= htmlspecialchars($resultado['Origem'] ?? 'N/A') ?>
                            </span>
                        </h3>
                        <p><strong>Código:</strong> <?= htmlspecialchars($resultado['Codigo'] ?? 'N/A') ?></p>
                        <p><strong>Descrição:</strong> <?= htmlspecialchars($resultado['Descricao'] ?? 'N/A') ?></p>
                    </div>
                <?php endforeach; ?>
            </div>
        <?php else: ?>
            <div class="no-resultados">
                <?php if (!empty($_GET['blocos']) || !empty($_GET['registros'])): ?>
                    Nenhum resultado encontrado com esses filtros. Tente ajustar os critérios!
                <?php else: ?>
                    Digite os termos de busca acima para começar sua pesquisa.
                <?php endif; ?>
            </div>
        <?php endif; ?>
    </main>
