...
$nome = isset($_GET['nome']) ? '%' . sanitize_input($_GET['nome']) . '%' : '%';
...
if ($preco_min > $preco_max) {
    // Se o preço mínimo for maior que o máximo, trocamos os valores
    $temp = $preco_min;
    $preco_min = $preco_max;
    $preco_max = $temp;
}
...
<head>
    ...
    <style>
        ...
        header {
            ...
            display: flex;
            align-items: center;
            justify-content: space-between; /* importante para separar esquerda e direita */
            padding: 0 40px;
...
<body>
    <header>
        <div class="auth-left">
            ...
            <?php else: ?>
                <a href="login.php" title="Entrar" class="login-icon">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" style="width:24px; height:24px;">
